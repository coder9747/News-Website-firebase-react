    ...
    return (
        <Layout>
            <div class="inline-flex items-center justify-center w-full">
                ...
                <span class="absolute px-3 font-medium text-xl text-gray-900 -translate-x-1/2 bg-white left-1/2 dark:text-white dark:bg-gray-900">{category} News</span>
            </div>
            <div className=' md:px-20 px-10 grid lg:grid-cols-4 gap-2 grid-cols-1 md:grid-cols-2 place-items-center'>
                {post && post.map((item) => {
                    return (<Link to={`/news/${item.id}`} class="md:max-w-sm max-w-xs bg-white border mx-auto border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div class="p-5">
                            <a href="#">
                                <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{item.category}</h5>
                            </a>
